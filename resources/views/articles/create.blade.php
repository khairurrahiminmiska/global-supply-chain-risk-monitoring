...
<div class="space-y-8">
    ...
    <div>
        <h1 class="text-4xl font-bold text-slate-800">Create Article</h1>
        <p class="text-gray-500 mt-2">Write a new analysis article</p>
    </div>
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="category"
                        ...
                        <option value="Analysis" @selected(old('category') === 'Analysis')>Analysis</option>
                        <option value="Market Report" @selected(old('category') === 'Market Report')>Market Report</option>
                        <option value="Risk Assessment" @selected(old('category') === 'Risk Assessment')>Risk Assessment</option>
                        <option value="Policy Update" @selected(old('category') === 'Policy Update')>Policy Update</option>
                        <option value="Industry News" @selected(old('category') === 'Industry News')>Industry News</option>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <textarea name="content" rows="16" required
                          class="w-full border border-slate-300 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:outline-none font-mono text-sm">{{ old('content') }}</textarea>
